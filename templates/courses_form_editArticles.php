...
<?php
	...
	if($crow['ARTICLES'] == ''){
		$crowArticles = "<empty></empty>";
	}else{
		$crowArticles = $crow['ARTICLES'];
	}
	
	$rootNode = $xmlDoc->loadXML($crowArticles);
	$rootNode = $xmlDoc->documentElement;
	
	if(!xmlHasNextNode(0,$crowArticles)){
	...
	foreach($rootNode->childNodes as $child){
		if($child->tagName == 'article'){
			...
			echo '<td>';
			echo "<a href=\"articles.php?f=editArticle&aid=" . $data['ID'] ." \"><img src=\"" . ICONS_PATH . "pencil.png\" alt=\"Edit\"/>Article</a>";
			echo '</td>';
			echo '<td>';
			echo "<a href=\"articles.php?f=admin_articlePages&aid=" . $data['ID'] ." \"><img src=\"" . ICONS_PATH . "pencil.png\" alt=\"Edit\"/>Pages</a>";
			echo '</td>';
			echo '<td>';
		...
		if($child->tagName == 'resource'){
			...
			if(check_user_permission("content_modify")){
				echo '<td>';
				echo "<a href=\"courses.php?f=editResource&cid=".$cid."&resid=" . $sid ." \"><img src=\"" . ICONS_PATH . "pencil.png\" alt=\"Edit\"/>Edit</a>";
				echo '</td>';
				echo '<td>';
				echo "<a href=\"index.php?action=mv_art&id=" . $cid ."&dir=up&aid=". $sid ." \"><img src=\"" . ICONS_PATH . "arrow_up.png\" alt=\"Move Up\"/></a>";
				echo "<a href=\"index.php?action=mv_art&id=" . $cid ."&dir=down&aid=" . $sid ." \"><img src=\"" . ICONS_PATH . "arrow_down.png\" alt=\"Move Down\"/></a>";
				echo '</td>';
			}
			if(check_user_permission("content_remove")){
				echo '<td>';
				$link = '<a href="courses.php?q=removeResource&id=' . $sid . '&cid=' . $_GET['id'] .' "><img src="' . ICONS_PATH . 'cancel.png" alt="Delete"/></a><br/>';
				echo $link;
				echo '</td>';
			}
			echo '</tr>';	
