<?php
	...
	if(isset($_GET['role'])){
	$new = false;
	$q = 'SELECT * FROM roles WHERE id="' . $_GET['role'] . '"';
	...
	if($new){
		echo '<form class="roleForm" name="addRole" method="post" action="index.php?action=newRoleItem">';
	}else{
		echo '<form class="roleForm" name="editRole" method="post" action="index.php?action=updateRoleItem&id=' . $_GET['role'] .  '">';
	}
